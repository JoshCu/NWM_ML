...
def get_cell_weights(raster, gdf):
    output = exact_extract(
        ...
        gdf,
        ["cell_id", "coverage"],
        include_cols=["NWIS_site_id"],
        output="pandas",
    )
    return output.set_index("NWIS_site_id")
...
def create_forcings(start_time: str, end_time: str, output_folder_name: str) -> None:
    ...
    gdf = gpd.read_parquet("/mnt/hdd/savalan/stations_with_stats.parquet").to_crs(projection)
    logger.info("Got projection from grid file")
    # drop any stations with an ID above 4E+14
    logger.info("Got gdf")
    ...
    # shutdown any running dask clients
